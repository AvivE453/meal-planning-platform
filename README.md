...
| External data | Edamam Food Database API | Live nutrition data instead of a maintained static database |
...
3. Split the daily target across slots (breakfast 25% / lunch 35% / dinner 30% / snack 10%) and, per slot, gather candidate `FoodItem`s from a small fixed set of search terms (e.g. dinner: `salmon`, `sweet potato`, `broccoli`), deduped by food id. Edamam's API is search-by-text with no "browse by meal type," so this fixed-term-list is a deliberate scope simplification for a portfolio project — a real product would let users pick preferred foods/cuisines or index a much larger catalog directly.
4. Filter candidates: the strategy's exclusion rules (`filterExcluded`, e.g. weight-loss excludes >20g sugar/serving) and a dietary-restriction filter (`filterByRestrictions`) that matches a user's allergy/restriction values against food names as a case-insensitive substring — crude (would also catch "buttermilk" for a "milk" restriction) but honest given Edamam's basic search response carries no structured allergen tags.
...
6. Persist inside one transaction: each selected food is upserted into the durable `food_items` table (keyed on `source` + `externalId` — not every search hit gets a row, only foods actually used in a plan, for FK integrity), then the `meal_plans`/`meal_plan_items` rows are written. The plan is re-read post-commit so the response and `GET /meal-plans` share one mapping function.

**A real free-tier constraint, found by live-verifying rather than assuming:** a cold-cache generation can issue up to 12 live Edamam calls (4 slots × 3 terms). The first live attempt against the real API failed with a `429` partway through — Edamam's free tier has a requests-per-minute budget well below that burst. The fix wasn't to guess at exact timing: `gatherCandidates` now runs each term sequentially (no self-inflicted concurrency spike) and, more importantly, catches and logs a per-term failure rather than letting one rate-limited or flaky term abort the whole plan — a slot just ends up with fewer candidates rather than the request failing outright. After the first successful generation, repeat runs mostly hit Redis (search results cache for 24h) and rarely touch Edamam's limit at all. Live-verified end-to-end afterward: a real `weight_loss` request generated a 24-item plan (real foods like "Chicken Breast", "Salmon Chum", "Broccoli Raab" with real brands and macros) totaling ~2313 kcal against a 2295 kcal target — comfortably inside the builder's 15% tolerance.

**Testing:** [`meal-plans.e2e-spec.ts`](apps/api/test/meal-plans.e2e-spec.ts) (6 tests against a real Postgres + a `FakeNutritionApiClient`, covering the 404 preconditions, a full generate → persist → list round trip, and newest-first ordering) and [`restriction-filter.spec.ts`](apps/api/src/meal-plans/restriction-filter.spec.ts) (4 unit tests for the substring-matching logic). The DP/Strategy/Builder logic itself is covered separately by `packages/algorithm`'s own 32 tests — these tests exist to prove the *wiring*, not re-prove the algorithm.
...
- [x] `POST /meal-plans/generate` wired end-to-end (Strategy → candidate search → exclusion/restriction filter → MCKP optimizer → Builder → Postgres) + `GET /meal-plans`, both live-verified against the real Edamam network; basic `MealPlanCard` on both web and mobile
